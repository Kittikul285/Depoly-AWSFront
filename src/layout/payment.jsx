...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        const response = await axios.get(import.meta.env.VITE_API + 'cart/showidcart', {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const token = localStorage.getItem('token');
        if (token) {
          const response = await axios.get(import.meta.env.VITE_API + 'cart/user', {
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  useEffect(() => {
    const fetchAddresses = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(import.meta.env.VITE_API + 'locations/getlocation', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const M = async () => {
    try {
      const cartcloneResponses = await Promise.all(
        cart.map(item => axios.post(import.meta.env.VITE_API + 'cart/cartclone/', { id: item.id }))
      );

      const rs1 = await axios.post(import.meta.env.VITE_API + 'orders/create', {
      ...
      const rqpay = await axios.post(import.meta.env.VITE_API + 'payment/payments', paymentData);
  ...
  const modelApiPayFn = async (paymentId) => {
    ...
    try {
      ...
      await axios.post(import.meta.env.VITE_API + 'payment/pay/', formData, {
        headers: {
  ...
  const updateProductStore = async (products) => {
    try {
      ...
      await axios.put(import.meta.env.VITE_API + `product/productstore/${products.product.id}`, {
        store: updatedStore
      });
    } catch (err) {
...
const LocationModal = ({ isOpen, onClose, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(import.meta.env.VITE_API + 'locations/location', formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      onClose();
...
const EditLocationModal = ({ isOpen, onClose, locationData, userId }) => {
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(import.meta.env.VITE_API + `locations/editlocation/${locationData.id}`, EditformData, {  // Ensure the correct endpoint and ID are used.
        headers: { Authorization: `Bearer ${token}` }
      });
      onClose();
