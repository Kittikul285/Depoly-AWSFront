...
export default function ListProduct() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const rs = await axios.get(import.meta.env.VITE_API + `product/getproduct`, {
  ...
  const hdldele = async (id) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const rs = await axios.delete(import.meta.env.VITE_API + `product/del/${id}`, {
        headers: {
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const rs = await axios.put(import.meta.env.VITE_API + `product/productedit/${ids}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
