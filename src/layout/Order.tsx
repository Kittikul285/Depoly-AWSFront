...
const Order = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios(import.meta.env.VITE_API + 'orders/orderallpay', {
  ...
  const updatePaymentStatus = async (paymentId, status) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(import.meta.env.VITE_API + "payment/putstatuspayments",{ paymentId, status },{ headers: { Authorization: `Bearer ${token}` } });
